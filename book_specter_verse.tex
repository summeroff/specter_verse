...
\usepackage{geometry}
\geometry{a4paper, margin=3cm} % Increased from 2.5cm
\usepackage{fancyhdr}
\setlength{\headheight}{15pt} % Fix header height
\addtolength{\topmargin}{-3pt} % Optional margin adjustment
\pagestyle{fancy}
\fancyhf{}
...
\usepackage{microtype} % Added for better typesetting
\usepackage{csquotes}
\usepackage{hyperref}
\hypersetup{
    colorlinks=true,
...
\begin{titlepage}
    \centering
    \vspace*{3cm}
    {\fontsize{48}{60}\selectfontSpecterVerse: The Endeavor Gambit\par} % Large, bold title
    \vspace{2cm}
